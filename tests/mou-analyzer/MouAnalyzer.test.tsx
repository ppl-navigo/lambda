...
describe("MouAnalyzer", () => {
  test("renders Navbar, Sidebar and Dropzone when no PDF is set", () => {
    render(<MouAnalyzer />);
    ...
    // Neither MarkdownViewer nor Streamer are rendered.
  ...
  test("switches to analysis view (MarkdownViewer & Streamer) when PDF is set", () => {
    ...
    expect(screen.getByTestId("markdownViewer")).toHaveTextContent(
      "http://test.pdf"
    );
    expect(screen.getByTestId("streamer")).toHaveTextContent(
      "http://test.pdf"
    );
  });
});
